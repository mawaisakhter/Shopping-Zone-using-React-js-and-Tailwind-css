...
const Products = () => {
    ...
    return (
        ...
        <section className="w-fit mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 md:gap-x-24 mt-10 mb-5">
            {filteredData.map((item, idx) => (
                <div className="w-72 bg-white shadow-md rounded-xl duration-500 z-0 hover:scale-105 hover:shadow-xl">
                    <Link to={`/ProductPage/${item.id}`}>
                        <img key={idx} src={item.ProductImage} alt="Product" className="h-80 w-72 object-cover rounded-t-xl" />
                    </Link>
